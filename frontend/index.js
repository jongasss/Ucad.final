...
(function () {
  ...
  async function loadPosts() {
    ...
    feed.innerHTML = 'Carregando posts...'; // Feedback visual

    try {
      const res = await fetch(api + '/posts');

      // VERIFICA SE A REQUISIÇÃO HTTP FALHOU (ex: status 500, 404)
      if (!res.ok) {
        // Tenta ler a mensagem de erro que o servidor enviou
        const errorData = await res.json().catch(() => null); // Tenta parsear o JSON de erro
        const message = errorData?.message || res.statusText;
        throw new Error(`Erro ao buscar posts: ${message}`);
      }
      ...
      if (!Array.isArray(posts)) {
        console.error("A API não retornou um array:", posts);
      ...
      // Agora é seguro iterar, pois 'posts' é um array
      for (const p of posts) {
        ...
        if (authorData && authorData.profile_picture_url) {
          const img = document.createElement('img'); img.src = api + "/uploads/" + authorData.profile_picture_url; img.style.width = '100%'; img.style.height = '100%'; img.style.objectFit = 'cover'; avatarWrap.appendChild(img);
        } else {
    ...
    <div style=\"color: #7c3aed; font-size: 13px; margin-top: 8px; font-weight: 600;\">Ver post completo →</div>`;
      ...
      // MOSTRA O ERRO NA TELA EM VEZ DE QUEBRAR O SCRIPT
      console.error('Falha ao carregar posts:', error);
      feed.innerHTML = `<p style="color: red;">Não foi possível carregar os posts. ${error.message}</p>`;
    }
  }
